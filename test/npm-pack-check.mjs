...
try {
  ...
  const need = [
    ...
    /* v1.5.10：插件自包含 runtime（ZCode 安装后 .mcp.json 指向的唯一入口） */
    'package/plugins/stepfun-usage-monitor/runtime/bin/cli.mjs',
    'package/plugins/stepfun-usage-monitor/runtime/proxy.mjs',
    'package/plugins/stepfun-usage-monitor/runtime/mcp-server.mjs',
    'package/plugins/stepfun-usage-monitor/runtime/stats.mjs',
    'package/plugins/stepfun-usage-monitor/runtime/dashboard.html',
    'package/plugins/stepfun-usage-monitor/runtime/package.json',
    'package/plugins/stepfun-usage-monitor/runtime/lib/paths.mjs',
    'package/plugins/stepfun-usage-monitor/runtime/lib/providers.mjs',
  ...
  ok('tarball 含全部运行文件（含 ZCode 插件结构 + runtime 自包含）', missing.length === 0, missing.length ? '缺 ' + missing.join(',') : `${need.length} 个文件`);
  ...
  ok('安装后插件 runtime/bin/cli.mjs 存在（自包含，安装不断链）', fs.existsSync(rtCli));
  if (fs.existsSync(rtCli)) {
    const rtOut = execFileSync(NODE, [rtCli, '--mcp'], {
      input: mcpIn, encoding: 'utf8', env: { ...process.env, DATA_DIR: dataTmp }, timeout: 15000,
    });
