...
const byDay = new Map(), byModel = new Map(), byAgent = new Map(), byProvider = new Map();
...
(async () => {
  ...
  for await (const chunk of stream) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      bump(byProvider, rec.provider || 'stepfun', p, c, t);   // v1.5.5：按服务商分组
  ...
  parentPort.postMessage({
    ok: true, start, end,
    totals, byDay: [...byDay], byModel: [...byModel], byAgent: [...byAgent], byProvider: [...byProvider], recents: ordered,
  });
